...
---boss系统初始化函数
---@param b lstg.GameObject @目标boss
...
function system:init(b, name, cards, bg, diff)
    ...
    for i = 1, b.sc_left do
        table.insert(_sc_left_type, 'normal')
    end
end
...
function system:checkAutoSPPoint()
    ...
    for i = #b._sp_point_auto, 1, -1 do
        ...
        if dmg >= p.dmg then
            _flag = true
        elseif p.current and p.timer and current_timer >= p.timer then
            _flag = true
        elseif not (p.current) and p.timer and timer >= p.timer then
            _flag = true
        end
        if _flag then
