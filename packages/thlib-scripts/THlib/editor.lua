...
function _straight:init(imgclass, index, x, y, v, angle, aim, omiga, stay, destroyable, time, _495, accel, accangle, maxv, through, bound)
    ...
    if bound ~= nil then self.bound = bound end
end
...
---@param bgmname boolean @是否在右下角显示bgm名称，默认显示
function _play_music(name, ptime, bgmname)
    if bgmname == nil then bgmname = true end
