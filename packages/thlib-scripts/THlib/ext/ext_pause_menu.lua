...
---！注意！
---暂停菜单中不能使用KeyIsDown或KeyIsPressed（因为KeyState停止更新），
---只能使用aic.input.CheckLastKey
...
ext.option = {
    ...
    username = loadConfigureTable().username,
    ...
    reload = function(self)
        ...
        self.lang_kt = setvaluetable(aic.table.Copy(aic.l10n.lang_list), lang)
    ...
    frame = function(self)
        ...
        if self.wait < 1 and not self.locked then
            ...
            if self.level == 1 then
                ...
                elseif aic.input.CheckLastKey('left') then
                    self.wait = 8
                ...
                elseif aic.input.CheckLastKey('right') then
                    self.wait = 8
                ...
                if self.key_changing then
                    ...
                    if aic.input.CheckLastKey('spell') or aic.input.CheckLastKey('menu') then
                        ...
                        self:flyout(-1)
                        self:flyin()
                    elseif aic.input.CheckLastKey('shoot') then
