...
---中点圆算法的简单实现，用来做弹幕的话要自己传入创建obj的函数
---不考虑效率问题的话上面那个椭圆拟合其实也够用，但是要做出那种锯齿感只能用这个
---@param x0 number 圆心x坐标
---@param y0 number 圆心y坐标
---@param r number 圆半径
---@param f fun(x:number, y:number) 绘制像素的函数
---@param minUnit number 像素单位大小
function lib.MidpointCircle(x0, y0, r, f, minUnit)
    -- 转换到像素坐标
    local cx = int(x0 / minUnit + 0.5)
    local cy = int(y0 / minUnit + 0.5)
    local r_pixels = r / minUnit

    local function DrawCirclePoints(x0, y0, x, y)
        f(x0 + x, y0 + y)
        f(x0 - x, y0 + y)
        f(x0 + x, y0 - y)
        f(x0 - x, y0 - y)
