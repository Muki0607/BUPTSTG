			...
			if self.timer%8<4 then
				...
				for i=1,4 do
					if self.sp[i] and self.sp[i][3]>0.5 then
						New(hifuu_bullet_heart,'hifuu_bullet_heart',self.supportx+self.sp[i][1],self.supporty+self.sp[i][2],8,self.anglelist[num][i],self.target,900,0.5)
					end
				end
			end
			...
			if self.support>0 then
				if self.timer%8<4 then
					for i=1,4 do
						if self.sp[i] and self.sp[i][3]>0.5 then
							for j=-2,2 do
								New(hifuu_bullet_renko_sub,'hifuu_bullet_renko_sub',self.supportx+self.sp[i][1],self.supporty+self.sp[i][2], 12, 90 + j*15 + j * 5 * sin(1.5 * self.timer), 0.3)
							end
						end
					end
				end
...
function hifuu_player:render()
	for i=1,4 do
		if self.sp[i] and self.sp[i][3]>0.5 then
			if self.slow == 1 then
				RenderAnimation('hifuu_support_merry',self.timer, self.supportx+self.sp[i][1],self.supporty+self.sp[i][2],0, 0.3)
			else
				RenderAnimation('hifuu_support_renko',self.timer, self.supportx+self.sp[i][1],self.supporty+self.sp[i][2],self.timer, 0.3)
			end
		end
	end
...
function hifuu_player:frame()
	...
	if self.bladeing then
		...
		for i,o in ObjList(GROUP_ENEMY) do
			if not o.spell_immune and o.colli and Dist(self,o) < self.blade_range + o.a then
		...
		for i,o in ObjList(GROUP_ENEMY_BULLET) do
			if not o.spell_immune and not o._blade_marked and Dist(self,o) < self.blade_range + o.a then
				PlaySound('slash',0.8)
...
function hifuu_bullet_renko_main_ef:init(x,y)
	self.x=x self.y=y self.rot=90 self.img='hifuu_bullet_renko_main_ef' self.layer=LAYER_PLAYER_BULLET+50 self.group=GROUP_GHOST
	self.vy=2.25
end
...
function hifuu_bullet_renko_sub_ef:init(x,y,rot)
	self.x=x self.y=y self.rot=rot self.img='hifuu_bullet_renko_sub_ef' self.layer=LAYER_PLAYER_BULLET+50 self.group=GROUP_GHOST
...
function hifuu_bullet_ef:init(x,y,rot)
	self.x=x self.y=y self.rot=rot self.img='hifuu_bullet_ef' self.layer=LAYER_PLAYER_BULLET+50 self.group=GROUP_GHOST
end
...
function hifuu_bullet_orange_ef:init(x,y,rot)
	self.x=x self.y=y+32 self.rot=rot self.img='hifuu_bullet_orange_ef' self.layer=LAYER_PLAYER_BULLET+50 self.group=GROUP_GHOST self.vy=2
...
function hifuu_bullet_orange_ef2:init(x,y)
	self.x=x self.y=y+32 self.rot=-90+ran:Float(-10,10) self.img='hifuu_bullet_orange_ef2' self.layer=LAYER_PLAYER_BULLET+50 self.group=GROUP_GHOST
...
function hifuu_bullet_heart_ef:init(x,y,rot)
	self.x=x self.y=y self.rot=rot self.img='hifuu_bullet_heart_ef' self.layer=LAYER_PLAYER_BULLET+50 self.group=GROUP_GHOST
	self.vx=1*cos(rot) self.vy=1*sin(rot)
end
